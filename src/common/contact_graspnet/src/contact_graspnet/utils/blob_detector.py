...
#from graspnet_detection import get_boxes, get_image
...
class BlobTracker(object):
    ...
    def apply_search_window(self, image, window_adim=None):
        """
        Apply search window
        return: image
        """
        if window_adim is None:
            window_adim = [0.0, 0.0, 1.0, 1.0]
        rows = image.shape[0]
        cols = image.shape[1]
        x_min_px = int(cols * window_adim[0])
        y_min_px = int(rows * window_adim[1])
    ...
    def blur_outside(self, image, blur=5, window_adim=None):
        """
...
if __name__ == "__main__":
    ...
    #raw_img = get_image()
    #print(raw_img)
    #cv_image = bridge_object.imgmsg_to_cv2(raw_img, desired_encoding="bgr8")
   ...
   # blob_detector_object.draw_keypoints(cv_image, keypoints, imshow=True)
    ...
    # [array([295.62607, 295.8693 , 353.58716, 357.95718], dtype=float32)]
    # [array([264.57672, 229.95422, 306.3087 , 325.53366], dtype=float32)]
    # [311.50607, 318.41434, 365.21957, 384.2404 ]
    #box = get_boxes()
    y1 = 311
    y2 = 365
    x1 = 318
    x2 = 384
    # crop image
    body_colours = cv_image[y1:y2, x1:x2]

    body_colourss = np.float32(body_colours.reshape(body_colours.size / 3, 3))

    # get dominant colour in LAB for colour name
    dominant_colour, top_colour, centres = k_means_colour(3, body_colourss)
    print(top_colour)
    ...
    cv2.imshow("body_colours 2", body_colours)
    cv2.waitKey(0)
